...
export default function MentorLogin() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // Check if the response is OK AND if the token exists
      if (res.ok && data.token) {
        // Correctly save the token from the server response
        localStorage.setItem("token", data.token);
        // Also save the mentorId for use in the dashboard
            ...
            <form onSubmit={handleLogin} className="space-y-6">
              <div>
                ...
                <input
                  ...
                  className="text-black w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-400 transition"
                  required
                />
              </div>
              ...
              <button
                type="submit"
                className="w-full flex items-center justify-center bg-green-500 text-white py-3 rounded-lg font-semibold hover:bg-green-600 transition-all duration-300 transform hover:scale-105"
              >
                <LoginIcon />
                Login
              </button>
